...
Instead of nerfing the generalist's baseline v(R_g) → v(R_g) - δ, we can create conditional vulnerabilities:
...
This solution is theoretically useful because it addresses the **root cause** (flattened best-response landscapes) rather than **symptoms** (dominant strategies). By making counterplay apparent the arbitor can:
